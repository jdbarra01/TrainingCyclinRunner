export function calculatePowerZones(ftp: number) {
  return {
    ...
    zona5: { min: Math.round(ftp * 1.06), max: Math.round(ftp * 1.2), name: 'VO2Máx' },
    zona6: { min: Math.round(ftp * 1.21), max: Math.round(ftp * 1.5), name: 'Anaérobico' },
  }
}

export function estimateFtpFromVO2max(vo2max: number, weight: number): number {
  return Math.round(vo2max * 0.22 * weight)
}
